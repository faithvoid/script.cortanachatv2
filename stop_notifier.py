...
def delete_pid():
    if os.path.exists(PID_FILE):
        os.remove(PID_FILE)
        xbmcgui.Dialog().ok("Stop Notifier", "Cortana Chat Notifier has been stopped.")
    else:
        xbmcgui.Dialog().ok("Stop Notifier", "PID file not found, the notifier might not be running!")
